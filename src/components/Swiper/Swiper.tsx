...
export class Swiper extends Component {
    ...
    */
    animate(elem , targetJSON , time , tweenString , callback){
        ...
        let timer = setInterval(() => {
            ...
            for(let k in originalJSON){
                // tween就表示这一帧应该在的位置：
                tween = tweenFunc[tweenString](frameNumber , originalJSON[k] , deltaJSON[k] , maxFrameNumber)
                //根据是不是opacity来设置单位
                if(k != 'opacity'){
    ...
    // 小圆点样式
    checkDots(index) {
        return index === this.state.activeIndex ? 'active' : 'dots'
    }
    ...
    gotoDotView() {
        if(this.state.dotsIndex == this.state.activeIndex){
            ...
            if(this.state.dotsIndex - this.state.activeIndex > 0){
                ...
                this.itemsArr.forEach((item, index) => {
                    let self = item
                    let nextIndex = index - Number(dotsDiff)
                    if(nextIndex < 0){
                ...
                this.itemsArr.forEach((item, index) => {
                    let self = item
                    let prevIndex = index + Number(dotsDiff)
                    if(prevIndex >= len){
    ...
    render() {
        ...
        return (
            <div className="poster-main" style={{width:this.props.lunboObject.width}} onMouseOver={this.mouseHandle.bind(this)} onMouseLeave={this.mouseHandle.bind(this)}>
                ...
                <ul className="poster-list" style={{width:this.props.lunboObject.width}}>
                   {
                        this.props.imgArray.map(function(item, index){
                            return <li ref={'items'+index} className="poster-item" style={this.renderstyle(index)} key={index}><a href={this.props.linkArray[index]}><img width="100%" height="100%" src={item}/></a></li>
                        }.bind(this))
                   }
                </ul>
